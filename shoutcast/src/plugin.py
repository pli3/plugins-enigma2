...
def Plugins(**kwargs):
	list = [PluginDescriptor(name="SHOUTcast", description=_("Internet radio"), where = [PluginDescriptor.WHERE_PLUGINMENU], icon="plugin.png", fnc=main)] # always show in plugin menu
	if config.plugins.shoutcast.showinextensions.value:
		list.append (PluginDescriptor(name="SHOUTcast", description=_("Internet radio"), where = [PluginDescriptor.WHERE_EXTENSIONSMENU], fnc=main))
	return list
